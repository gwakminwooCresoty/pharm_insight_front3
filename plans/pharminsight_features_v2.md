...
> **주요 변경** 멀티 테넌트 URL 구조 · 역할 기반 접근 제어(RBAC) · 플랫폼 관리자 API · Spring Pageable 페이지네이션
...
2. [멀티 테넌시 아키텍처](#2-멀티-테넌시-아키텍처)
   ...
   - 2-6. [테넌트 격리 메커니즘](#2-6-테넌트-격리-메커니즘)
...
| 전버전 대비 변경 | v1.0 단일 테넌트 구조 → 멀티 테넌트 URL 체계, RBAC, 플랫폼 관리자 API, Spring Pageable 규격 전면 반영 |
...
PharmInsight는 **3계층 테넌트 구조**로 운영된다. 각 계층은 완전히 격리되며, 상위 계층은 하위 계층을 조망할 수 있다.

```
Platform  (플랫폼 전체)
├── Franchise A  (프랜차이즈 / 브랜드 단위 — 테넌트 격리 기본 단위)
...
> **핵심 원칙**: `franchiseId`가 테넌트 격리의 기본 단위이다. 모든 데이터는 `franchise_id` 컬럼으로 파티셔닝되며, 어떤 경로로도 타 프랜차이즈 데이터에 접근할 수 없다.
...
# 2. 프랜차이즈 범위 — franchiseId로 테넌트 격리
...
| `TENANT_MANAGE` | 테넌트 관리 | `PLATFORM_ADMIN` 전용 |
...
| 6 | 플랫폼 | FUNC-PLAT-002 | 테넌트 관리 (CRUD) | `PLATFORM_ADMIN` | 플랫폼 관리 > 테넌트 |
...
| 403 Forbidden | 권한 없음 (타 테넌트 접근, 역할 미충족, storeId 불일치) |
...
| 정책 | 설명 |
|------|------|
| 테넌트 격리 | `@TenantFilter` AOP로 모든 쿼리에 `franchise_id` 자동 주입. 코드 레벨 누락 방지 |
