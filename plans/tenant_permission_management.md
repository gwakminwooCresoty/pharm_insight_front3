...
## 1. 개요
플랫폼 관리자에서 각 테넌트(고객사, 프랜차이즈 본사, 약국 등)별로 접근 가능한 메뉴와 기능의 권한을 효율적이고 유연하게 제어하기 위한 권한 부여 체계(Authorization Model) 기획안입니다.
...
* **개념**: 테넌트 생성 시 기본적으로 접근이 허용되어야 하는 최소한의 필수 메뉴 집합입니다.
* **동작**: 시스템 설정에 'DEFAULT' 역할로 미리 메뉴들이 세팅되어 있으며, 테넌트 상세 관리 화면에서 토글 버튼(`[ON/OFF]`)을 통해 일괄 적용하거나 해제할 수 있습니다.
* **장점**: 모든 테넌트에게 부여해야 하는 공통 권한을 매번 세팅할 필요가 없어 관리자 리소스가 절약됩니다.
...
* **동작**: 여러 단일 메뉴를 하나하나 선택할 필요 없이, 미리 만들어진 권한 그룹을 체크박스로 다중 선택(Multi-select)하여 테넌트에게 부여합니다. 
...
* **동작**: 플랫폼 관리자 화면의 **[권한 상세 설정 (트리 뷰)]**에서 체크박스를 클릭하여 특정 테넌트에게만 메뉴를 열어주거나(Add) 닫을 수(Remove) 있습니다.
...
사용자(테넌트 소속)가 메뉴에 접근할 때 시스템이 권한을 최종적으로 판단하는 공식은 다음과 같습니다. 합집합 방식으로 권한을 결합하고, 명시적 제외(Deny)를 우선 처리합니다.
...
해당 기능을 UI로 풀어낼 때의 화면 구성도 제안입니다. (테넌트 상세 > [권한 설정] 탭)
...
3. **상세 개별 권한 제어 (Advanced Permission Override)**
   ...
   * **[시각적 상태 구분]**:
     ...
     * 🟥 **빨간색 X 상태 (Blocked)**: 그룹 권한에는 포함되어 있으나, 이 테넌트에 한해서만 명시적으로 '차단'시킨 메뉴.
...
* `tenant_role_mappings` (테넌트에게 부여된 역할 그룹들 연결)
* `tenant_menu_exceptions` (테넌트 개별 수준에서의 예외 처리)
  * 필드: `tenant_id`, `menu_id`, `is_granted` (true면 개별 추가 허용, false면 그룹 상관없이 강제 차단)
