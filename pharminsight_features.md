...
| 3 | 매장 선택 | MultiSelect Dropdown | 전체 또는 특정 매장 선택 (멀티 테넌트) |
...
| 403 Forbidden | 권한 없음 (타 테넌트 데이터 접근 시도 등) |
...
| 정책 | 설명 |
|------|------|
| 테넌트 격리 | JWT 토큰 내 `tenantId` 기반으로 데이터 자동 필터링. 타 테넌트 데이터 접근 불가 |
...
| Rate Limiting | 테넌트별 분당 100 요청 제한 |
